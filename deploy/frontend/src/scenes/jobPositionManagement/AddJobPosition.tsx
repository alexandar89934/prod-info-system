...
import { addJobPosition } from '@/state/jobPosition/jobPosition.actions.ts';
import {
...
const AddJobPosition = () => {
  ...
  const {
    ...
    control,
    formState: { errors },
  } = useForm<AddJobPositionFormData>({
  ...
  const onSubmit = async (data: AddJobPositionFormData) => {
    await dispatch(addJobPosition(data)).unwrap();
    reset();
  ...
  return (
    ...
    >
      ...
      >
        <Header title={t('jobPosition.form.addTitle')} subtitle="" />
        ...
        >
          ...
          <Box display="flex" gap={2}>
            ...
            >
              {loading ? t('jobPosition.form.addSubmitLoading') : t('jobPosition.form.addSubmit')}
            </Button>
            <Button
              variant="outlined"
              onClick={onCancel}
