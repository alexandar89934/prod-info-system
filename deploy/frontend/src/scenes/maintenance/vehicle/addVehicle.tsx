...
import { addVehicle } from '@/state/vehicle/vehicle.actions';
import { selectVehicleError, selectVehicleLoading } from '@/state/vehicle/vehicle.selectors';
import { clearError } from '@/state/vehicle/vehicle.slice';
...
const AddVehicle = () => {
  ...
  const onSubmit = async (data: VehicleFormData) => {
    ...
    if (addVehicle.fulfilled.match(result)) navigate('/vehicle');
  };
  ...
  return (
    <Box display="flex" justifyContent="center" alignItems="center" height="100%" sx={{ p: 2 }}>
      ...
      >
        <Typography variant="h4" align="center" mb={2}>{t('vehicle.add')}</Typography>
        ...
        <Box display="flex" gap={2} mt={3}>
          <Button type="submit" variant="contained" disabled={loading}
            ...
            {loading ? `${t('vehicle.add')}...` : t('vehicle.add')}
          </Button>
