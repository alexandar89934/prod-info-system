...
import { fetchItemById, updateItem } from '@/state/item/item.actions';
import {
  selectCurrentItem,
  selectItemError,
  selectItemLoading,
...
import { EditItemFormData } from '@/state/item/item.types';
...
const EditItem = () => {
  ...
  useEffect(() => {
    if (currentItem) {
      ...
      if (currentItem.pictures) {
        setPictures(
          currentItem.pictures.map((p) => ({
            name: p.name,
            path: p.path,
            dateAdded:
              p.dateAdded instanceof Date
                ? p.dateAdded.toISOString()
                : p.dateAdded,
          }))
        );
      }
      if (currentItem.documents) {
        setDocuments(
          currentItem.documents.map((d) => ({
            name: d.name,
            path: d.path,
            dateAdded:
              d.dateAdded instanceof Date
  ...
  }, [currentItem, reset, setPictures, setDocuments]);
      ...
      >
        ...
        >
          ...
          <Controller
            ...
            render={({ field, fieldState }) => (
              <LabeledXtSelect
                id="approvalLevel"
                label={t('item.form.approvalLevel')}
                value={field.value ?? ''}
                onChange={(e) => field.onChange(e.target.value || null)}
                error={fieldState.error}
                options={ITEM_APPROVAL_LEVELS.map((l) => ({
                  value: l,
                  label: t(`item.approvalLevel.${l}`),
                }))}
          ...
          <Controller
            ...
            render={({ field, fieldState }) => (
              <LabeledXtSelect
                ...
                options={molds.map((m) => ({
                  value: m.id ?? '',
                  label: `#${m.inventoryNumber} — ${m.name}`,
                }))}
          ...
          <Controller
            name="priceEurPerUnit"
            control={control}
            render={({ field }) => (
              <LabeledXtField
                id="priceEurPerUnit"
                label={`${t('item.form.priceEurPerUnit')} (€)`}
                type="number"
                error={errors.priceEurPerUnit}
                {...field}
                value={field.value ?? ''}
                onChange={(e) =>
                  field.onChange(
        ...
        >
          ...
          <Box display="flex" gap={2}>
            ...
            >
              {loading
                ? t('item.form.editSubmitLoading')
                : t('item.form.editSubmit')}
            </Button>
            <Button
              variant="outlined"
