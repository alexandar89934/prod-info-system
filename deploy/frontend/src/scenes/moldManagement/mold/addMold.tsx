...
import { AddMoldFormData } from '@/state/mold/mold.types';
...
const AddMold = () => {
  ...
  const onSubmit = async (data: AddMoldFormData) => {
    const result = await dispatch(
      addMold({
        ...data,
        pictures: picturesToSubmit,
        documents: documentsToSubmit,
      })
    );
    if (addMold.fulfilled.match(result)) {
      reset();
      setPictures([]);
      setDocuments([]);
      navigate('/mold');
  ...
  return (
    ...
    >
      ...
      >
        <Header title={t('mold.form.addTitle')} subtitle="" />
        ...
        >
          ...
          <Controller
            ...
            render={({ field }) => (
              <LabeledXtField
                id="inventoryNumber"
                label={t('mold.form.inventoryNumber')}
                type="number"
          ...
          <Controller
            ...
            render={({ field, fieldState }) => (
              <LabeledXtSelect
                id="status"
                label={t('mold.form.status')}
                value={field.value}
                onChange={(e) => field.onChange(e.target.value)}
                error={fieldState.error}
                options={[
                  { value: 'ok', label: t('mold.status.ok') },
                  { value: 'fault', label: t('mold.status.fault') },
                  { value: 'repair', label: t('mold.status.repair') },
                ]}
          ...
          <Controller
            ...
            render={({ field, fieldState }) => (
              <LabeledXtSelect
                id="serviceCategory"
                label={t('mold.form.serviceCategory')}
                value={field.value ?? ''}
                onChange={(e) => field.onChange(e.target.value || null)}
                error={fieldState.error}
                options={['S-1', 'S-2', 'S-3', 'S-4'].map((v) => ({
                  value: v,
                  label: v,
                }))}
          ...
          />
          <Controller
            name="centeringDiameterMM"
            control={control}
            render={({ field }) => (
              <LabeledXtField
                id="centeringDiameterMM"
                label={`${t('mold.form.centeringDiameterMM')} (mm)`}
                type="number"
                error={errors.centeringDiameterMM}
                {...field}
                value={field.value ?? ''}
                onChange={(e) =>
                  field.onChange(
          ...
          />
          <Controller
            name="requiredClampingForceKN"
            control={control}
            render={({ field }) => (
              <LabeledXtField
                id="requiredClampingForceKN"
                label={`${t('mold.form.requiredClampingForceKN')} (kN)`}
                type="number"
                error={errors.requiredClampingForceKN}
                {...field}
                value={field.value ?? ''}
                onChange={(e) =>
                  field.onChange(
          ...
          <Box display="flex" flexDirection="column" gap={1}>
            {fields.map((field, index) => (
              ...
              >
                <Box flex={2} minWidth={0}>
                  <Controller
                    name={`temperingTemperatures.${index}.zone`}
                    control={control}
                    render={({ field: f }) => (
                      <LabeledXtField
                        id={`zone-${index}`}
                        label={t('mold.form.zone')}
                        minWidth="60px"
                        {...f}
                        value={f.value ?? ''}
                      />
                ...
                <Box flex={1} minWidth={0}>
                  <Controller
                    name={`temperingTemperatures.${index}.minTemp`}
                    control={control}
                    render={({ field: f }) => (
                      <LabeledXtField
                        id={`minTemp-${index}`}
                        label={`${t('mold.form.minTemp')} (°C)`}
                        type="number"
                        minWidth="90px"
                        {...f}
                        value={f.value ?? ''}
                ...
                <Box flex={1} minWidth={0}>
                  <Controller
                    name={`temperingTemperatures.${index}.maxTemp`}
                    control={control}
                    render={({ field: f }) => (
                      <LabeledXtField
                        id={`maxTemp-${index}`}
                        label={`${t('mold.form.maxTemp')} (°C)`}
                        type="number"
                        minWidth="90px"
                        {...f}
                        value={f.value ?? ''}
            ...
            <Button
              ...
              onClick={() => append({ zone: '', minTemp: 0, maxTemp: 0 })}
          ...
          <Controller
            ...
            render={({ field }) => (
              <LabeledXtField
                id="pieceCounter"
                label={t('mold.form.pieceCounter')}
                type="number"
                error={errors.pieceCounter}
                {...field}
                value={field.value ?? 0}
        ...
        >
          ...
          <Box display="flex" gap={2}>
            ...
            >
              {loading
                ? t('mold.form.addSubmitLoading')
                : t('mold.form.addSubmit')}
            </Button>
            <Button
              variant="outlined"
              onClick={() => navigate('/mold')}
              disabled={loading}
              sx={{
